...
single_path_heuristic_evidence_penalty::single_path_heuristic_evidence_penalty(ListDigraph& wc,
        ListDigraph::Node& s,
        ListDigraph::Node& t,
...
single_path_heuristic_evidence_penalty::~single_path_heuristic_evidence_penalty() {
}



void single_path_heuristic_evidence_penalty::extract_transcripts(alternative_transcript_collection& results) {
...
void single_path_heuristic_evidence_penalty::find_min_max(std::deque<ListDigraph::Arc> &path) {
...
//                    logger::Instance()->debug("Skip \n");
                ...
                if (p_it->second.penalty + added_penalty < penalty  || (p_it->second.penalty + added_penalty == penalty && p_it->second.cap > max) ) {
                    max = p_it->second.cap;
                    max_arc = arc;
...
//        logger::Instance()->debug("Test " + std::to_string(p_it->first) + " " + std::to_string(p_it->second.back_node) + " " + std::to_string(wc.id(p_it->second.arc)) + " " + std::to_string(p_it->second.cap) + "\n");
        if (p_it->second.penalty < penalty || ( p_it->second.penalty == penalty && p_it->second.cap > max) ) {
