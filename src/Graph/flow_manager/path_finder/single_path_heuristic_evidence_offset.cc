...
single_path_heuristic_evidence_offset::single_path_heuristic_evidence_offset(ListDigraph& wc,
        ListDigraph::Node& s,
        ListDigraph::Node& t,
...
single_path_heuristic_evidence_offset::~single_path_heuristic_evidence_offset() {
}



void single_path_heuristic_evidence_offset::extract_transcripts(alternative_transcript_collection& results) {
...
void single_path_heuristic_evidence_offset::find_min_max(std::deque<ListDigraph::Arc> &path) {
...
//        
    ...
    unsigned int max_offset = size + 5;
    ...
    for (std::deque<ListDigraph::Node>::iterator n = top_order.begin(); n!= top_end; ++n, ++i ) {
        ...
        for (ListDigraph::OutArcIt a(wc,*n); a!= INVALID; ++a) {
            ...
            capacity_type max = 0;
            unsigned int offset = max_offset;
            capacity_type arc_cap = 0;
            rcount evidence = 0;
            ...
            for (dyn_paths::iterator p_it = min_max[i].begin(); p_it != min_max[i].end(); ++p_it) {
                ...
                rcount added_evidence = 0;
                if (!kp[arc].is_evidenced_path()) {
                    added_evidence = kp[arc].get_path_evidence(wc.id(a));
                }
                
                logger::Instance()->debug("Offset " + std::to_string(offset) + "\n");
                logger::Instance()->debug("TestMax " + std::to_string(p_it->second.offset) + " " + std::to_string(p_it->second.cap) + "\n");
                
                if (p_it->second.offset  < offset
                    || (p_it->second.offset == offset && p_it->second.cap > max) 
                    || (p_it->second.offset == offset && p_it->second.cap == max && p_it->second.arc_cap > arc_cap) 
                   ) {
                    max = p_it->second.cap;
                    max_arc = arc;
                    offset = p_it->second.offset;
                    arc_cap = p_it->second.arc_cap;
                    evidence = std::min(p_it->second.evidence, added_evidence);
            ...
            if (max == 0) {
                ...
                if (cet[a] == edge_types::HELPER) {
                    ListDigraph::OutArcIt next(wc,wc.target(a));
                    while (cet[next] != edge_types::EXON && cet[next] == ces[next].node_index == -1 ) ++next;
                    if (cet[next] == ces[next].node_index != -1) {
                        offset = ces[next].node_index;
                    } else {
                        offset = ces[next].id.find_first();
                    }
                } else {
                    if (ces[a].node_index != -1) {
                        offset = ces[a].node_index;
                    } else {
                        offset = ces[a].id.find_first();
                    }
                }
            ...
            if (wc.target(a) == t) {
                if (cet[a] == edge_types::HELPER) {
                    ListDigraph::InArcIt next(wc,wc.source(a));
                    while (cet[next] != edge_types::EXON && ces[next].node_index == -1) ++next;
                    if (ces[next].node_index != -1) {
                        offset += size - ces[next].node_index -1;
                    } else {
                        boost::dynamic_bitset<>::size_type index = ces[next].id.find_first();
                        unsigned int end_index = index;
                        while(index != boost::dynamic_bitset<>::npos) {
                                end_index = index;
                                index = ces[next].id.find_next(index);
                        }
                        offset += size - end_index - 1;
                    }
                } else {
                    if (ces[a].node_index != -1) {
                        offset += size - ces[a].node_index -1;
                    } else {
                        boost::dynamic_bitset<>::size_type index = ces[a].id.find_first();
                        unsigned int end_index = index;
                        while(index != boost::dynamic_bitset<>::npos) {
                                end_index = index;
            ...
            new_path.arc_cap = cfc[a];
            new_path.offset = offset;
            new_path.evidence = evidence;
            
            logger::Instance()->debug("Added " + std::to_string(offset) + " " + std::to_string(min_cap) + " " + std::to_string(wc.id(max_arc)) + "\n");
    ...
    for (int j = 0; j <= i; j++) {
        ...
        for (dyn_paths::iterator p_it = min_max[j].begin(); p_it != min_max[j].end(); ++p_it) { 
            logger::Instance()->debug("E " + std::to_string(p_it->first) + " " + std::to_string(p_it->second.back_node) + " " + std::to_string(wc.id(p_it->second.arc)) + " off " + std::to_string(p_it->second.offset) + " larc " + std::to_string(p_it->second.arc_cap)  + " max " + std::to_string(p_it->second.cap) + "\n");
        }
    }
    ...
    for (dyn_paths::iterator p_it = min_max[index].begin(); p_it != min_max[index].end(); ++p_it) { 
        ...
        logger::Instance()->debug("Test " + std::to_string(p_it->first) + " " + std::to_string(p_it->second.back_node) + " " + std::to_string(wc.id(p_it->second.arc)) + " off " + std::to_string(p_it->second.offset) + " larc " + std::to_string(p_it->second.arc_cap)  + " max " + std::to_string(p_it->second.cap) + "\n");
        if (p_it->second.offset  < offset
                    || (p_it->second.offset == offset && p_it->second.cap > max) 
                    || (p_it->second.offset == offset && p_it->second.cap == max && p_it->second.arc_cap > arc_cap) 
                   ) {  
